...
export default function TenantAdminDashboard() {
  ...
  const getRoleName = (type: string) => {
    const types: { [key: string]: string } = { 
      report: '業務日報', crm: '商談メモ', incident: 'ヒヤリハット', inspection: '現調記録', braindump: 'アイデア' 
    }
        ...
        <div className="p-4">
          ...
          <nav className="space-y-1.5">
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition ${activeTab === 'settings' ? 'bg-slate-800 text-indigo-400' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
            >
            ...
            <div className="space-y-6 max-w-4xl">
              {loading ? (
                ...
                <div className="text-center py-20 text-slate-500 border border-dashed border-slate-800 rounded-2xl">
                  <p className="font-bold">まだ提出された日報はありません。</p>
                </div>
              ) : (
                reports.map((report) => (
                        ...
                        {report.status === 'pending' ? (
                          <div className="flex flex-col h-full">
                            ...
                            <div className="flex gap-2">
                              <button 
                                ...
                                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-2.5 rounded-lg text-sm font-bold transition flex items-center justify-center gap-2"
                              >
                                <CheckCircle2 className="w-4 h-4" />
                                承認して完了
                              </button>
                          ...
                          <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-xl p-4 h-full flex flex-col">
                            <p className="text-sm text-indigo-200 leading-relaxed grow">
                              {report.feedback || 'フィードバックなしで処理されました。'}
                            </p>
                            <button 
            ...
            <div className="max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl p-6">
              ...
              <div className="flex gap-3 mb-8">
                <input
                  ...
                  placeholder="新しい項目名（例：業務内容、申し送り事項...）"
                  className="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-4 text-sm text-slate-200 focus:outline-none focus:border-indigo-500"
                />
                <button
                  onClick={handleAddField}
                  disabled={isAddingField || !newFieldName.trim()}
                  className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 transition"
                >
                  {isAddingField ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                  追加
                </button>
              </div>
              ...
              <div className="space-y-3">
                {fields.length === 0 ? (
                  <div className="text-center py-8 text-slate-500 border border-dashed border-slate-700 rounded-xl">
                    項目が登録されていません。<br/>デフォルトではユーザーは自由に発話します。
                  </div>
                ) : (
                  fields.map(field => (
